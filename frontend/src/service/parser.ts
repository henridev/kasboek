...
export const createRowForDataOnlyXLS = (sheet: Sheet): [Sales, Cards, Cheques] => {
    ...
    while (verkoopCijfers[1] < verkoopCijfers[2]) {
      ...
      const val = sheet[keyForVal]?.v?.v;
      const titel = sheet[keyForTitel]?.v?.v;
      verkoopCijfers[1]++;
    ...
    const cells = Object.entries(sheet)
      ...
      .map(([, val]) => (typeof val?.v?.v === "string" ? genKey(val?.v?.v) : val?.v?.v));
  ...
  export const createRowForCompleyXLS = (sheet: Sheet): [Sales, Cards, Cheques] => {
    const sales = newSales();
